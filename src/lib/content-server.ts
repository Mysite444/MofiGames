...
export async function getPostsByTag(tagSlug: string): Promise<PublicPost[]> {
  try {
    ...
    const { data, error } = await withTimeout(
      supabase.from("posts").select("*").in("id", postIds).eq("is_published", true).is("deleted_at", null).order("published_at", { ascending: false }),
      DEFAULT_SUPABASE_TIMEOUT_MS,
...
/**
 * Returns up to `limit` published posts that share at least one tag with
 * the given post, excluding the post itself.  Used by the "Related Posts"
 * section on /blog/[slug].
 *
 * Strategy: collect the tag IDs of the current post → find all post_tags
 * rows with those IDs (excluding the current post) → deduplicate post IDs →
 * fetch those posts → attach their tags.
 *
 * Falls back to [] on any error so a DB hiccup never breaks a post page.
 */
export async function getRelatedPosts(currentPostId: string, tagIds: string[], limit = 3): Promise<PublicPost[]> {
  if (tagIds.length === 0) return [];
  try {
    const supabase = createPublicClient();

    // 1. Find post IDs that share at least one tag (exclude the current post)
    const { data: links } = await withTimeout(
      ...
      DEFAULT_SUPABASE_TIMEOUT_MS,
      "getRelatedPosts/post_tags"
    );

    if (!links || links.length === 0) return [];

    // 2. Deduplicate and take the most-overlap IDs first (order by frequency)
    const freq = new Map<string, number>();
    for (const { post_id } of links) {
      freq.set(post_id, (freq.get(post_id) ?? 0) + 1);
    }
    const ranked = [...freq.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, limit * 3) // fetch more than needed so we have buffer after is_published filter
      .map(([id]) => id);

    // 3. Fetch the actual post rows (published only)
    const { data: rows, error } = await withTimeout(
      supabase
        .from("posts")
        .select("id, slug, title, excerpt, cover_image_url, author_name, published_at, seo_title, seo_description, seo_canonical_url, seo_focus_keyword, seo_secondary_keywords, seo_h1_title, seo_index, og_title, og_description, og_image_url, og_image_alt, twitter_card")
        .in("id", ranked)
    ...
    // Re-sort by tag-overlap rank
    const sorted = rows
      .sort((a, b) => (freq.get(b.id) ?? 0) - (freq.get(a.id) ?? 0))
      .slice(0, limit);

    const tagsByPost = await attachTags(sorted.map((r) => r.id));
    ...
    return sorted.map((data) => ({
      ...
      title: data.title,
      excerpt: data.excerpt,
      content: "",           // not needed for card rendering
      coverImageUrl: data.cover_image_url,
      authorName: data.author_name,
      publishedAt: data.published_at,
      tags: tagsByPost.get(data.id) ?? [],
    ...
    if (isNextControlFlowError(err)) throw err;
    console.error("[content-server] getRelatedPosts failed, returning []:", err);
    return [];
  }
}
