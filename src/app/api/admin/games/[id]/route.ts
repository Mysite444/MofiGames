...
/** DELETE /api/admin/games/:id — admin only. Tag links clean up via
 * `on delete cascade` on game_tags; Storage files (thumbnail, cover
 * image, trailer, preview video, loading screen, uploaded build) don't
 * have a DB-level cascade — Storage is a separate system from Postgres —
 * so they're explicitly removed below via deleteGameStorageFiles(). */
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // only record of which Storage objects belong to this game.
  const { data: existing } = await supabase
    .from("games")
    .select(
      "title, slug, thumbnail_url, cover_image_url, video_trailer_url, preview_video_url, loading_screen_url, storage_path"
    )
  ...
  let cleanup: { removed: number; errors: string[] } = { removed: 0, errors: [] };
  if (existing) {
    cleanup = await deleteGameStorageFiles(supabase, existing);
  ...
  await logAdminAction(supabase, user, {
    ...
    summary: existing ? `Deleted game "${existing.title}" (${existing.slug}).` : "Deleted a game.",
    metadata: existing
      ? { title: existing.title, slug: existing.slug, filesRemoved: cleanup.removed, storageErrors: cleanup.errors }
      : {},
  });
