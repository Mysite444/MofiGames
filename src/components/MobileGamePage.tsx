...
export function MobileGamePage({
  ...
  return (
    <div className="flex flex-col gap-4 -mt-3 pb-[calc(6rem+env(safe-area-inset-bottom))] lg:hidden">
      {/* Hero — the whole area is the actual video/play frame now (edge to
          edge, no rounding), with a soft fade to black at the bottom so it
          blends into the content below instead of cutting off sharply. The
          rounded thumbnail badge sits centered on top of it, same as the
          reference, until the game is started. */}
