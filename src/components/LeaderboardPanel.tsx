  ...
  return (
    <section className="relative overflow-hidden">
      ...
      {/* Desktop/laptop — same tile (GenreGameCard, 202x114) and sizing used
          ...
          pl-5/scroll-pl-5 — same "moved left" left inset now used on every
          desktop row (CategoryRow, ContinuePlayingDesktop, TopPicksRow),
          still a hair above GenreGameCard's ~18.2px hover-grow clearance
          need (see CategoryRow.tsx for the full math). scroll-pl-5 matches
          it so scroll-snap doesn't eat it on the first card (same fix as
          CategoryRow/TopPicksRow). The trailing spacer is widened to match,
          so the last card's right-side glow has room too. */}
      <div className="relative mt-2 hidden gap-3.5 overflow-x-auto pt-3 pb-4 pl-5 scroll-pl-5 scrollbar-hide snap-rail lg:flex">
        {games.map((g) => (
