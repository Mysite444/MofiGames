...
export function BrowseGameCard({ game, category }: { game: Game; category: Category | undefined }) {
  ...
  return (
    <Link
      ...
      onBlur={stopPreview}
    >
      <div className="tile-shine relative aspect-video w-full overflow-hidden rounded-thumb ring-1 ring-white/10 transition-all duration-300 ease-tile group-hover:scale-[1.22] group-hover:ring-2 group-hover:ring-[var(--color-cta-blue)] group-hover:shadow-[0_0_22px_2px_rgba(var(--color-cta-blue-rgb),0.55),0_18px_38px_rgba(0,0,0,0.6)] group-focus-visible:scale-[1.22] group-focus-visible:ring-2 group-focus-visible:ring-[var(--color-cta-blue)] group-focus-visible:shadow-[0_0_22px_2px_rgba(var(--color-cta-blue-rgb),0.55),0_18px_38px_rgba(0,0,0,0.6)] group-active:scale-[0.97] group-active:duration-150 group-active:ease-out">
        {imageSrc ? (
        ...
        {game.tag && BadgeIcon && (
          <span
            className={`absolute left-1.5 top-1.5 flex items-center gap-0.5 rounded px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wide transition-opacity duration-300 ease-tile group-hover:opacity-0 group-active:opacity-0 ${badgeStyles[game.tag]}`}
          >
        ...
        {/* CrazyGames-style hover info panel — title, genre tag, plays,
            ...
            Uses the site's own blue accent (--color-cta-blue) for the panel
            fill, matching the glow ring above, instead of a plain black
            fade. */}
        <div className="pointer-events-none absolute inset-x-0 bottom-0 flex translate-y-1.5 flex-col gap-1 bg-gradient-to-t from-[var(--color-cta-blue)] from-60% to-transparent px-2 pb-1.5 pt-2.5 opacity-0 transition-all duration-300 ease-tile group-hover:translate-y-0 group-hover:opacity-100 group-focus-visible:translate-y-0 group-focus-visible:opacity-100">
