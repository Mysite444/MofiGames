...
/** Matches GenreGameCard's fixed-aspect rounded-thumb tile used in every
 * desktop rail (202x114, ~16:9). Parent controls the actual size. */
export function GameTileSkeleton({ className = "" }: { className?: string }) {
  return <Bone className={`h-full w-full rounded-thumb ${className}`} />;
}
...
export function CategoryPageCardSkeleton() {
  return <Bone className="aspect-video w-full rounded-thumb" />;
}
...
export function FeaturedBannerSkeleton() {
  return (
    <div className="relative aspect-[16/9] w-full overflow-hidden rounded-thumb ring-1 ring-white/10">
      <Bone className="absolute inset-0 h-full w-full rounded-thumb" />
