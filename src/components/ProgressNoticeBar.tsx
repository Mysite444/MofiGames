 ...
 * the game frame, not a floating pill), with a solid black background —
 * same #000 used for the fullscreen player chrome and header elsewhere in
 * the app — so the warning reads clearly over any game art underneath it.
 * White warning text and buttons are centered together as one cluster
 * rather than split to the edges. Slides in on mount with a soft glow that
 * breathes gently to keep it noticeable without being obnoxious. Kept
 * compact (~40px tall) so it doesn't eat into the game view.
 */
export function ProgressNoticeBar({ onClose }: { onClose: () => void }) {
  return (
    <div className="notice-bar-in notice-bar-glow flex w-full items-center justify-center gap-3 bg-black px-3.5 py-2 sm:px-4">
