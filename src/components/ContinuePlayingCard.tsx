...
export function ContinuePlayingCard({ game }: { game: Game }) {
  ...
  return (
    ...
    >
      ...
      {game.tag && (
        <span
          className={`absolute left-1.5 top-1.5 rounded px-1.5 py-0.5 text-[9px] font-bold tracking-wide transition-opacity duration-100 group-hover:opacity-0 ${tagStyles[game.tag]}`}
        >
          {game.tag}
        </span>
      )}
      ...
      <div className="pointer-events-none absolute inset-x-1.5 bottom-1.5 flex translate-y-1 flex-col gap-0.5 opacity-0 transition-all duration-100 group-hover:translate-y-0 group-hover:opacity-100 group-focus-visible:translate-y-0 group-focus-visible:opacity-100">
