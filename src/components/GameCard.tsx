...
export function GameCard({ game, hideTitle = false }: { game: Game; hideTitle?: boolean }) {
  ...
  return (
    <Link
      ...
      onBlur={stopPreview}
    >
      <div className="tile-shine relative aspect-square w-full overflow-hidden rounded-thumb ring-1 ring-white/10 transition-all duration-300 ease-tile group-hover:scale-[1.03] group-hover:ring-2 group-hover:ring-[var(--color-cta-blue)] group-hover:shadow-[0_0_20px_2px_rgba(var(--color-cta-blue-rgb),0.55),0_6px_20px_rgba(0,0,0,0.4)] group-focus-visible:ring-2 group-focus-visible:ring-[var(--color-cta-blue)] group-focus-visible:shadow-[0_0_20px_2px_rgba(var(--color-cta-blue-rgb),0.55),0_6px_20px_rgba(0,0,0,0.4)] group-active:scale-[0.97] group-active:duration-150 group-active:ease-out">
        {imageSrc ? (
        ...
        {game.tag && (
          <span
            className={`absolute left-2 top-2 rounded-md px-1.5 py-0.5 text-[10px] font-bold tracking-wide transition-opacity duration-300 ease-tile group-hover:opacity-0 group-active:opacity-0 ${tagStyles[game.tag]}`}
          >
            {game.tag}
          </span>
        )}
