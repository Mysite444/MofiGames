...
export async function deleteGameStorageFiles(
  ...
  // Cover image / trailer / preview video / loading screen / three new
  // purpose-specific covers all share the game-media bucket — one batched
  // remove() call for all of them.
  const mediaPaths = [
    pathFromBlobUrl(game.cover_image_url, "game-media"),
    pathFromBlobUrl(game.landscape_cover_url, "game-media"),
    pathFromBlobUrl(game.square_cover_url, "game-media"),
    pathFromBlobUrl(game.portrait_cover_url, "game-media"),
...
const GAME_SELECT_COLUMNS =
  "thumbnail_url, cover_image_url, landscape_cover_url, square_cover_url, portrait_cover_url, video_trailer_url, preview_video_url, loading_screen_url, storage_path";
...
export async function scanOrphanedGameMedia(supabase: SupabaseClient): Promise<OrphanScanResult> {
  ...
  for (const g of rows) {
    ...
    for (const url of [
      g.cover_image_url,
      g.landscape_cover_url,
      g.square_cover_url,
      g.portrait_cover_url,
      g.video_trailer_url,
      g.preview_video_url,
      g.loading_screen_url,
    ]) {
